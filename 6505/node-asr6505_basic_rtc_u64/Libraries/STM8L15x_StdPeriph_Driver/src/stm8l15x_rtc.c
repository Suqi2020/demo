...
uint16_t RTC_GetSubSecond(void)
{
  ...
  return (uint16_t)(ssrreg&0x7ff);//suqi
...
#ifndef  ADD_NEWCODE
//////////////////////////////////////////////////SUQI
uint16_t RTC_SetSubSecond(uint16_t RTC_SubSecondValue)//需要先设置ms 再设置年月日时分秒 因ms中有进位操作
{
  uint16_t subsec=0;
  RTC_SubSecondValue=RTC_SubSecondValue&0x7ff;
  subsec=RTC_GetSubSecond();//获取当前值
  subsec = subsec&0x7ff;
 // if(subsec>RTC_SubSecondValue)
  
   // RTC_SynchroShiftConfig(RTC_ShiftAdd1S_Set,2048-subsec+RTC_SubSecondValue);  //2048-RTC_SubSecondValue  error  subsec-RTC_SubSecondValue
   // printf("a\r\n");
  
 // else

    RTC_SynchroShiftConfig(RTC_ShiftAdd1S_Reset,RTC_SubSecondValue-subsec);
    //printf("b\r\n");

  return ERROR;
}
#if 0
uint16_t RTC_SetSubSecond_backup(uint16_t RTC_SubSecondValue)//需要先设置ms 再设置年月日时分秒 因ms中有进位操作
{
  uint16_t subsec=0;
  uint16_t subsecRead;
  RTC_SubSecondValue=RTC_SubSecondValue&0x7ff;
                  ...
                  RTC_AlarmTypeDef* RTC_AlarmStruct)
{
  ...
  RTC->ALRMAR3 = tmpreg3;
  RTC->ALRMAR4 = tmpreg4;
//printf("%d %d %d %d\r\n",(unsigned char)tmpreg1,(unsigned char)tmpreg2,(unsigned char)tmpreg3,tmpreg4);
  /* Enable the write protection for RTC registers */
  RTC->WPR = 0xFF; 
}
                                   ...
                                   uint16_t RTC_ShiftSubFS)
{
  ...
  while (((RTC->ISR1 & RTC_ISR1_SHPF) != RESET));//立刻读取需要加上 等待
      /* Read DR3 register to unfreeze calender registers */
   // (void)(RTC->DR3);//suqi

    /* Exit Initialization mode */
   // RTC->ISR1 &= (uint8_t)~RTC_ISR1_INIT;//suqi
  /* Enable the write protection for RTC registers */
  RTC->WPR = 0xFF; 
