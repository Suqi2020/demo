...
/**
  ...
  * @retval None
  */
u8 gu8Buffer[LENTH],gu8RecFlag=0;
u16 Len=0;
INTERRUPT_HANDLER(TIM2_CAP_IRQHandler, 20)//串口2
{
    //u8 test=0;
     // test=(USART_ReceiveData8 (USART1));
	...
	{ 
		  ...
		  //disableInterrupts();
		  gu8Buffer[Len] = USART_ReceiveData8(USART2);
		  if(gu8Buffer[Len]==0x0a)//收到一包完整數據
		  {
			gu8RecFlag=1;
		  }
		  Len++;
