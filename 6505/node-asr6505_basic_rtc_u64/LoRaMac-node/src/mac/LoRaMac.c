...
//volatile int32_t RtcTimeOffset=0;  //有符号型  可正可负
static TimerSysTime_t  LastTxSysTime={0};
static TimerSysTime_t  sysTimeAns ={0};
static TimerSysTime_t  sysSetTime={0};
static TimerSysTime_t  sysTimeCurrent={0};
...
static void OnRadioTxDone( void )
{
    ...
    TimerTime_t curTime = TimerGetCurrentTime();//ms tick获取当前时间�?tick-ms
   
    
    
#ifndef ADD_NEWCODE 
//    LastTxSysTime.Seconds= RtcGetTickCurrentTime()>>N_PREDIV_S;//此处获取的应该是没有修正过的ms级tick�?
//    LastTxSysTime.SubSeconds =RtcConvertTickToMs(RtcGetTickCurrentTime()&0x7ff);
#endif
...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN: {
          printf("test4\r\n");
            ...
            if ( isMicOk == true ) {
                ...
                printf("size  %d %d\r\n\r\n",size,appPayloadStartIndex); // 18 19
                // Process payload and MAC commands
                if ((signed int) ( ( size - 4 ) - appPayloadStartIndex ) > 0 ) {
                    ...
                    if ( fCtrl.Bits.FOptsLen > 0 ) {
                       ...
                       printf("test12\r\n");
                        ProcessMacCommands( payload, 8, appPayloadStartIndex, snr , McpsIndication.RxSlot);//执行此处
                    }
                }
    ...
    {// Procedure is completed when the AckTimeoutTimer is not running anymore
      ...
      printf("test7\r\n");
    	// Trig OnMacCheckTimerEvent call as soon as possible
    	TimerSetValue( &MacStateCheckTimer, 1 );//OnMacStateCheckTimerEvent
    	TimerStart( &MacStateCheckTimer );
    }
}
...
static void OnRxWindow2TimerEvent( void )
{
...
#if 0
  #ifdef TEST
     GPIO_SetBits(GPIOC, GPIO_Pin_1);//暂时放到 RegionCommon.c中测�?rxbeacon开窗时�?
  #endif
#endif
    TimerStop( &RxWindowTimer2 );
#if 0//suqi
    RxWindow2Config.Channel = Channel;
    RxWindow2Config.Datarate = DR;//suqi
    ...
    RxWindow2Config.Channel = Channel;//suqi
    RxWindow2Config.DrOffset = LoRaMacParams.Rx1DrOffset;
    RxWindow2Config.DownlinkDwellTime = LoRaMacParams.DownlinkDwellTime;
    RxWindow2Config.RepeaterSupport = LoRaMacParams.RepeaterSupport;
    RxWindow2Config.RxContinuous = false;
    RxWindow2Config.RxSlot = RxSlot;
    
    RxWindow2Config.Datarate =DR_TEST;//suqi
    ...
    switch ( cmd ) {
        ...
        case MOTE_MAC_DEV_STATUS_ANS:
            if ( MacCommandsBufferIndex < ( bufLen - 2 ) ) {
                ...
                status = LORAMAC_STATUS_OK;
#ifdef LORAMAC_CLASSB_TESTCASE
                printf("ready to send MOTE_MAC_DEV_STATUS_ANS p1=%d p2=%d\r\n",p1,p2);
#endif
            }
            break;
...
#ifndef ADD_NEWCODE             
/*
TimerTime_t currentTime = 0;  //U64
TimerSysTime_t sysTimeAns = { 0 };  //U32 SEC  U16 SUBSEC
...
#ifdef LORAMAC_CLASSB_TESTCASE
                printf("receive SRV_MAC_DEV_STATUS_REQ\r\n");
#endif
...
//#ifndef ADD_NEWCODE             
//                  
//                 // printf("tim:%x %x %x %x  %x\r\n",payload[macIndex],payload[macIndex+1],payload[macIndex+2],payload[macIndex+3],payload[macIndex+4]);
...
//                    sysTimeAns.SubSeconds = (int16_t)(sysTimeAns.SubSeconds * 3.90625); //一个字节扩大成2个字�?
//                  //  printf("gw time1:%ld %d\r\n",sysTimeAns.Seconds,sysTimeAns.SubSeconds);
...
//                    
//                    
//                    sysTimeCurrent.Seconds= RtcGetTickCurrentTime()>>N_PREDIV_S;//此处获取的应该是没有修正过的ms级tick�?
//                    sysTimeCurrent.SubSeconds =RtcConvertTickToMs(RtcGetTickCurrentTime()&0x7ff);
//                    
//
//                    sysSetTime=TimerAddSysTime(sysTimeCurrent,TimerSubSysTime(sysTimeAns,LastTxSysTime));
//                    RTC_AlarmCmd(DISABLE);//加入关中�?
//                    RtcWriteTick(sysSetTime.Seconds, RtcConvertMsToTick(sysSetTime.SubSeconds));//更新RTC时间
//                    RTC_AlarmCmd(ENABLE);//加入开中断
// #ifdef TEST
//    GPIO_ResetBits(GPIOC, GPIO_Pin_1);//设置�?测试设置完RTC到rxconfig的时�?
//#endif                    
//                   // RtcCalendar_t      test = RtcGetCalendar();//获取日历對比
...
//
//#ifdef LORAMAC_CLASSB_TESTCASE
//                    printf("receive SRV_MAC_DEVICE_TIME_ANS, set time=%u.%d\r\n", (unsigned int)sysSetTime.Seconds, sysSetTime.SubSeconds);
//
//#endif
//
//
//					
//                    uint32_t currentTime = TimerGetCurrentTime( );//获取u32 类型带ms——tick测试  测试  测试
//                    
//                    
//                    LoRaMacClassBDeviceTimeAns( currentTime );//同步 确认下一次开窗时�? BeaconCtx.NextBeaconRx
#ifdef CONFIG_LWAN                    
...
//    }
    ...
    RegionComputeRxWindowParameters( LoRaMacRegion,
                                     ...
                                     LoRaMacParams.SystemMaxRxError,
                                     &RxWindow1Config );
    LoRaMacParams.Rx2Channel.Datarate=RegionApplyDrOffset( LoRaMacRegion, LoRaMacParams.DownlinkDwellTime, LoRaMacParams.ChannelsDatarate,
                                                          LoRaMacParams.Rx1DrOffset );//suqi
    ...
    //5000 6000 4984  5984   (5975)
  //  printf("%ld %ld %ld %ld %d\r\n",LoRaMacParams.JoinAcceptDelay1,LoRaMacParams.JoinAcceptDelay2,RxWindow1Delay,RxWindow2Delay,LoRaMacParams.Rx2Channel.Datarate);
...
LoRaMacStatus_t SendFrameOnChannel( uint8_t channel )
{
    TxConfigParams_t txConfig;
    int8_t txPower = 0;
    LoRaMacParams.ChannelsDatarate=DR_TEST;//先固�? suqi  
    txConfig.Channel = channel;
    txConfig.Datarate = LoRaMacParams.ChannelsDatarate;
    txConfig.TxPower = LoRaMacParams.ChannelsTxPower;
    txConfig.MaxEirp = LoRaMacParams.MaxEirp;
...
//    AggregatedTimeOff = 0;
    ...
    LoRaMacClassBInit( &classBParams, &classBCallbacks );//死掉
