...
/*!
 * Defines preamble time
 */
uint16_t D2DCADPreambleTime = LORAWAN_PREAMBLE_TIME; 
/*!
 * LoRaMac Class D CAD timers
 */
TimerEvent_t D2DCADTimer;    
#endif
...
///////////////////////////////////////////////////////////数据发送
static char tx_cmd_str[LORAWAN_APP_DATA_MAX_SIZE];//数据发送
//AT+DRX=34,801377066680000002fa5621c2e7d3a079
static int conv_payload_to_atcmd( uint16_t size, uint8_t *payload)
{
    int i;
    int sz;
    int len;
    char *buf_p = tx_cmd_str;

    if (size > 250) {
        return -1;
    }

    sz = sprintf(buf_p,"AT+DRX=%u,",size*2);
    buf_p += sz;

    for (i = 0; i < size; i++) {
        sz = sprintf(buf_p, "%02x", payload[i]);
        buf_p += sz;
    }
...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
    ...
    if(gu8Join_bz==2)
    {
	    length = conv_payload_to_atcmd(size,payload);//suqi 发送给模块
	    scgw_uart_send(tx_cmd_str,length);//接收完了就发送
    }
#ifndef TEST //输出脉冲信号给mcu
  ...
  if( LoRaMacClassBRxBeacon( payload, size ) == true )
    ...
    switch( macHdr.Bits.MType )
    {
        case FRAME_TYPE_JOIN_ACCEPT:
            ...
            LoRaMacJoinDecrypt( payload + 1, size - 1, LoRaMacAppKey, LoRaMacRxPayload + 1 );//解密
            ...
            if( micRx == mic )
            {
                ...
                LoRaMacParams.ReceiveDelay1 *= 1000;//初始化RECEIVE_DELAY1时间窗口  1s
                LoRaMacParams.ReceiveDelay2 = LoRaMacParams.ReceiveDelay1 + 1000;//初始化RECEIVE_DELAY1时间窗口 2s
                ...
                printf("nwkskey: ");
                for(i=0; i<16; i++) {
                    printf("%02x ", (uint16_t)LoRaMacNwkSKey[i]);//得到两个会话秘钥
                }
                printf("\r\n");   
                
                printf("appskey: ");
                for(i=0; i<16; i++) {
                    printf("%02x ", (uint16_t)LoRaMacAppSKey[i]);//得到两个会话秘钥
                }
                printf("\r\n"); 
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            {
                ...
                if(d2dlocal){
                    ...
                    LoRaMacComputeMic( payload, size - LORAMAC_MFR_LEN, nwkSKey, address, dir, downLinkCounter, &mic );
                ...
                #endif
                {
                    ...
                    else
                    {
                        ...
                        LoRaMacComputeMic( payload, size - LORAMAC_MFR_LEN, nwkSKey, address,  dir, downLinkCounterTmp, &mic );
                        if( micRx == mic )
                        {
                            isMicOk = true;
                    ...
                    if( ( ( size - 4 ) - appPayloadStartIndex ) > 0 )
                    {
                        ...
                        if( port == 0 )
                        {
                            ...
                            if( fCtrl.Bits.FOptsLen == 0 )
                            {
                                ...
                                if(fCtrl.Bits.FOptsLen>0){
                                // Decode frame payload MAC commands 处理mac命令
                        ...
                        if( ( fCtrl.Bits.FOptsLen > 0 ) && ( multicast == 0 ) ){
                            ...
                            McpsIndication.Buffer = LoRaMacRxPayload;//真实数据？
...
{ 
 ...
 #ifdef TEST
	//GPIO_ResetBits(GPIOC, GPIO_Pin_2);for(int i=0;i<50;i++);GPIO_SetBits(GPIOC, GPIO_Pin_2);
...
/*
    ...
    #ifdef CONFIG_LINKWAN_D2D			
    else if(RxSlot == RX_SLOT_WIN_CLASS_D){
        LoRaMacState &= ~LORAMAC_RX;
    }
    #endif     
    else
...
static void OnRadioCadDone( bool channelActivityDetected )
{
    LoRaMacClassDCADDone(channelActivityDetected);
}
#endif

static void OnMacStateCheckTimerEvent( void )
{
    #ifndef CONFIG_SINGLE_CHANNEL
    GetPhyParams_t getPhy;
    PhyParam_t phyParam;
    #endif
    ...
    if( LoRaMacFlags.Bits.MacDone == 1 )
    {
        ...
        {// Retransmissions procedure for confirmed uplinks
            ...
            if( ( AckTimeoutRetriesCounter < AckTimeoutRetries ) && ( AckTimeoutRetriesCounter <= MAX_ACK_RETRIES ) )
            {
                ...
                //printf(">>>send 1\r\n");//此处中断唤醒后失败情况下多次发送
                if( ScheduleTx( ) == LORAMAC_STATUS_OK )//去掉数据发送失败重发机制
				//if(0)
                {
                    LoRaMacFlags.Bits.MacDone = 0;
                }
    ...
    if( LoRaMacState == LORAMAC_IDLE )
    {
        ...
        if( LoRaMacFlags.Bits.MlmeReq == 1 )
        {
            ...
            LoRaMacPrimitives->MacMlmeConfirm( &MlmeConfirm );//调用初始化的 LoRaMacPrimitives.MacMlmeConfirm = mlmeconfirm;
        }
...
#endif
#endif
        #ifdef CONFIG_SINGLE_CHANNEL 
        LoRaMacParams.ChannelsDatarate = DR_5;
        #else
...
{
  #ifdef TEST
	GPIO_SetBits(GPIOC, GPIO_Pin_0);//for(int i=0;i<50;i++);GPIO_SetBits(GPIOC, GPIO_Pin_0);
...
{
  #ifdef TEST
	GPIO_SetBits(GPIOC, GPIO_Pin_1);//for(int i=0;i<50;i++);GPIO_SetBits(GPIOC, GPIO_Pin_1);
#endif
    TimerStop( &RxWindowTimer2 );
    ...
    switch( cmd )
    {
       ...
       case MOTE_MAC_DEVICE_TIME_REQ:
            if( MacCommandsBufferIndex < LORA_MAC_COMMAND_MAX_LENGTH )
            {
               ...
               status = LORAMAC_STATUS_OK;
#ifdef LORAMAC_CLASSB_TESTCASE
                printf("ready to send MOTE_MAC_DEVICE_TIME_REQ\r\n");
#endif
            }
            break;
        case MOTE_MAC_PING_SLOT_INFO_REQ:
            if( MacCommandsBufferIndex < ( LORA_MAC_COMMAND_MAX_LENGTH - 1 ) )
            {
                ...
                status = LORAMAC_STATUS_OK;
#ifdef LORAMAC_CLASSB_TESTCASE
                printf("ready to send MOTE_MAC_PING_SLOT_INFO_REQ value=%d\r\n",p1);
#endif
            }
            break;
        ...
        case MOTE_MAC_BEACON_FREQ_ANS:
            ...
            break;
		#ifdef CONFIG_LINKWAN_D2D
        case MOTE_MAC_PREAMBLE_INFO_REQ:
            if( MacCommandsBufferIndex < LORA_MAC_COMMAND_MAX_LENGTH )
            {
                MacCommandsBuffer[MacCommandsBufferIndex++] = cmd;
                // Status: Channel frequency OK
    ...
    while( macIndex < commandsSize )
    {
        ...
        switch( payload[macIndex++] )
        {
            case SRV_MAC_LINK_CHECK_ANS:
                ...
                break;
			#ifdef CONFIG_LINKWAN_D2D
		case SRV_MAC_PREAMBLE_INFO_ANS:
                {
                    MlmeConfirm.Status = LORAMAC_EVENT_INFO_STATUS_OK;
                    MlmeConfirm.PreambleStatus = payload[macIndex++];

                    printf("\n\rLoRa:received SRV_MAC_PREAMBLE_INFO_ANS,PreambleStatus=%d\r\n",MlmeConfirm.PreambleStatus);
                    break;
                }                
			#endif                
            ...
            case SRV_MAC_DEVICE_TIME_ANS://suqi  临时屏蔽
                {
            ...
            case SRV_MAC_PING_SLOT_INFO_ANS:
                {
                    ...
                    {
#ifdef LORAMAC_CLASSB_TESTCASE
                        printf("receive SRV_MAC_PING_SLOT_INFO_ANS in RX1\r\n");
#endif
...
#ifdef LORAMAC_CLASSB_TESTCASE
                    printf("receive SRV_MAC_PING_SLOT_CHANNEL_REQ frequency=%u datarate=%u\r\n", (unsigned int)frequency, datarate);
#endif
            ...
            case SRV_MAC_BEACON_FREQ_REQ:
                {
                    ...
                    frequency *= 100;
#ifdef LORAMAC_CLASSB_TESTCASE
                    printf("receive SRV_MAC_BEACON_FREQ_REQ frequency=%u\r\n", (unsigned int)frequency);
#endif
...
LoRaMacStatus_t Send( LoRaMacHeader_t *macHdr, uint8_t fPort, void *fBuffer, uint16_t fBufferSize)
{
    ...
    status = PrepareFrame( macHdr, &fCtrl, fPort, fBuffer, fBufferSize );//打包
...
LoRaMacStatus_t PrepareFrame( LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    ...
    uint32_t destAddr = LoRaMacDevAddr;
    uint8_t *nwkSKey = LoRaMacNwkSKey;
    uint8_t *appSKey = LoRaMacAppSKey;
    ...
    switch( macHdr->Bits.MType )
    {
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_UP:
            ...
            adrNext.AdrEnabled = fCtrl->Bits.Adr; //传入prepaframe之前  fCtrl->Bits.Adr=AdrCtrlOn
			...
			#ifdef CONFIG_LINKWAN_D2D
            if (fPort == LORAWAN_LOCAL_APP_PORT){

                MulticastParams_t *curMulticastParams =  MulticastChannels;

                if ( curMulticastParams != NULL ) {
                    destAddr = curMulticastParams->Address;
                    nwkSKey = curMulticastParams->NwkSKey;
                    appSKey = curMulticastParams->AppSKey;
                }else{
                    #ifdef LORAMAC_CLASSB_TESTCASE
                    printf("no multicast channel found\r\n");
                    #endif
                    return LORAMAC_STATUS_SERVICE_UNKNOWN;
                }
            }
            ...
            LoRaMacBuffer[pktHeaderLen++] = ( destAddr ) & 0xFF;
            LoRaMacBuffer[pktHeaderLen++] = ( destAddr >> 8 ) & 0xFF;
            LoRaMacBuffer[pktHeaderLen++] = ( destAddr >> 16 ) & 0xFF;
            LoRaMacBuffer[pktHeaderLen++] = ( destAddr >> 24 ) & 0xFF;
            ...
            if( ( payload != NULL ) && ( LoRaMacTxPayloadLen > 0 ) )
            {
                if( MacCommandsInNextTx == true )
                {
                    if( MacCommandsBufferIndex <= LORA_MAC_COMMAND_MAX_FOPTS_LENGTH )
                    {
                        ...
                        {
                            LoRaMacBuffer[pktHeaderLen++] = MacCommandsBuffer[i];//mac命令放到FPort中传输
                        }
                    }
                    else
                    {
                        LoRaMacTxPayloadLen = MacCommandsBufferIndex;
                        payload = MacCommandsBuffer;//mac放入payload中传输 FPort为0独立帧格式
                        framePort = 0;
                    }
                }
            }
            ...
            if( ( payload != NULL ) && ( LoRaMacTxPayloadLen > 0 ) )
            {
                ...
                if( framePort == 0 )
                {
                    ...
                    MacCommandsBufferIndex = 0;
                    LoRaMacPayloadEncrypt( (uint8_t* ) payload, LoRaMacTxPayloadLen, nwkSKey, destAddr, UP_LINK, UpLinkCounter, &LoRaMacBuffer[pktHeaderLen] );
                }
                else
                {
                    LoRaMacPayloadEncrypt( (uint8_t* ) payload, LoRaMacTxPayloadLen, appSKey, destAddr, UP_LINK, UpLinkCounter, &LoRaMacBuffer[pktHeaderLen] );
                }
            }
            LoRaMacBufferPktLen = pktHeaderLen + LoRaMacTxPayloadLen;

            LoRaMacComputeMic( LoRaMacBuffer, LoRaMacBufferPktLen, nwkSKey, destAddr, UP_LINK, UpLinkCounter, &mic );
...
//	int i=0;
    ...
    txConfig.Datarate =g_sf_tx;
	//txConfig.Datarate =g_sf_tx;// txConfig.Datarate = LoRaMacParams.ChannelsDatarate;
 ...
 #ifdef TEST
		//GPIO_SetBits(GPIOE, GPIO_Pin_7);//SUQI
#endif
                
               
    Radio.Send( LoRaMacBuffer, LoRaMacBufferPktLen );//发送 调用RadioSend( uint8_t *buffer, uint8_t size )
	/*printf("Data Send1：");
	for(int i=0;i<LoRaMacBufferPktLen;i++)
	{
		printf("%x ",LoRaMacBuffer[i]);
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks, LoRaMacRegion_t region )
{
    ...
    // Initialize timers
    TimerInit( &MacStateCheckTimer, OnMacStateCheckTimerEvent );//回调  发送数据（非入网包）失败后一直发送
    TimerSetValue( &MacStateCheckTimer, MAC_STATE_CHECK_TIMEOUT );

    TimerInit( &TxDelayedTimer, OnTxDelayedTimerEvent );//入网失败反复发送
    TimerInit( &RxWindowTimer1, OnRxWindow1TimerEvent );//接收窗口处理1
    TimerInit( &RxWindowTimer2, OnRxWindow2TimerEvent );//接收窗口处理2
    TimerInit( &AckTimeoutTimer, OnAckTimeoutTimerEvent );
#ifdef CONFIG_LINKWAN_D2D
    TimerInit( &D2DCADTimer, OnD2DCADTimerEvent);
#endif  
    // Store the current initialization time
    LoRaMacInitializationTime = TimerGetCurrentTime( );

    // Initialize Radio driver
    RadioEvents.TxDone = OnRadioTxDone;//发送完成
    RadioEvents.RxDone = OnRadioRxDone;//接收完成
...
#endif    
    Radio.Init( &RadioEvents );//调用void RadioInit( RadioEvents_t *events ) RadioEvents = events;
...
#endif
#ifdef CONFIG_LINKWAN_D2D		
        case MIB_PREAMBLE_TIME:{
            mibGet->Param.PreambleTime = D2DCADPreambleTime;
            break;
        }
#endif	
        default:
        ...
        case MIB_DEVICE_CLASS:
        {
            switch( LoRaMacDeviceClass )
            {
                case CLASS_A:
                {
                    ...
                    #ifdef CONFIG_LINKWAN_D2D
                    else if(mibSet->Param.Class == CLASS_D)
                    {               
                        LoRaMacDeviceClass = mibSet->Param.Class;
...
#ifdef CONFIG_LINKWAN_D2D
        case MIB_PREAMBLE_TIME:{
            D2DCADPreambleTime = mibSet->Param.PreambleTime;
            break;
        }
#endif	
        default:
...
LoRaMacStatus_t LoRaMacMlmeRequest( MlmeReq_t *mlmeRequest )
{
    ...
    switch( mlmeRequest->Type )
    {
        ...
        case MLME_PREAMBLE_INFO:{
            uint8_t value = mlmeRequest->Req.PreambleInfo.Period.Value;
            ...
            // LoRaMac will send this command piggy-pack
            status = AddMacCommand( MOTE_MAC_PREAMBLE_INFO_REQ, value, 0 );
            break;
        }
#endif	  
