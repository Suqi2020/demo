 ...
 */
 void RxBeaconSetup( TimerTime_t rxTime, bool activateDefaultChannel )
{
...
//
//    RegionRxBeaconSetup( *LoRaMacClassBParams.LoRaMacRegion, &rxBeaconSetup, &LoRaMacClassBParams.McpsIndication->RxDatarate );//开窗设置
//    printf("start rx beacon: %ld, %u, (%llu), rxtime(%u)\r\n", frequency, LoRaMacClassBParams.McpsIndication->RxDatarate, TimerGetCurrentTime( ), (unsigned int)rxBeaconSetup.RxTime);
...
static void GetTemperatureLevel( LoRaMacClassBCallback_t *callbacks, BeaconContext_t *beaconCtx )
{
    ...
    {
        beaconCtx->Temperature = callbacks->GetTemperatureLevel( );//suqi hardware
    }
}
...
{
  TimerTime_t beaconEventTime = {0};//suqiTimerTime_t beaconEventTime = 0;
...
{
    return 128 / ( 1 << periodicity );  //periodicity  0-7
}
...
//                BeaconCtx.BeaconTime |= ( ( uint32_t )( ( uint32_t )payload[phyParam.BeaconFormat.Rfu1Size + 1] << 8 ) ) & 0x0000FF00;
//                BeaconCtx.BeaconTime |= ( ( uint32_t )( ( uint32_t )payload[phyParam.BeaconFormat.Rfu1Size + 2] << 16 ) ) & 0x00FF0000;
//                BeaconCtx.BeaconTime |= ( ( uint32_t )( ( uint32_t )payload[phyParam.BeaconFormat.Rfu1Size + 3] << 24 ) ) & 0xFF000000;
...
{
#ifdef LORAMAC_CLASSB_ENABLED
    PingSlotCtx.PingNb = CalcPingNb( periodicity );               //取值 总共能接收128 64 32 16 8 4 2 1 个时隙
    PingSlotCtx.PingPeriod = CalcPingPeriod( PingSlotCtx.PingNb );//取值 每隔32 64 128 256 512 1024 2048 4096个*30ms周期接收一次ping时隙      4096/PingSlotCtx.PingNb 
#endif // LORAMAC_CLASSB_ENABLED
}
