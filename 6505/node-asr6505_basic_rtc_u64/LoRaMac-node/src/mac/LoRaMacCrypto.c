...
void LoRaMacBeaconComputePingOffset( uint32_t beaconTime, uint32_t address, uint16_t pingPeriod, uint16_t *pingOffset )
{
    uint8_t zeroKey[16];
    uint8_t buffer[16];
    uint8_t cipher[16];
...
void LoRaMacD2DMcastComputeSKeys( const uint8_t *key, uint32_t mcAddr, uint8_t *nwkSKey, uint8_t *appSKey )
{
    ...
    compBase[0] = 0x01;
    compBase[1] = mcAddr & 0xFF;
    compBase[2] = ( mcAddr >> 8 ) & 0xFF;
    compBase[3] = ( mcAddr >> 16 ) & 0xFF;
    compBase[4] = ( mcAddr >> 24 ) & 0xFF;
    aes_encrypt( compBase, nwkSKey, &AesContext );
    ...
    compBase[3] = ( mcAddr >> 16 ) & 0xFF;
    compBase[4] = ( mcAddr >> 24 ) & 0xFF;
    aes_encrypt( compBase, appSKey, &AesContext );
}
