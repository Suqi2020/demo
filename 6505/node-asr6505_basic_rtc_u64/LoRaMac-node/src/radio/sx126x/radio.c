                         ...
                         bool iqInverted, bool rxContinuous )
{
    ...
    switch( modem )
    {
        ...
        case MODEM_LORA:
            SX126xSetStopRxTimerOnPreambleDetect( false );
          // printf(">>>%d\r\n",(int)symbTimeout);//symbTimeout检测前导码超时时间  SUQI
...
void RadioIrqProcess( void )
{
    if( IrqFired == true )
    {
        ...
        if( ( irqRegs & IRQ_TX_DONE ) == IRQ_TX_DONE )
        {
            ...
            {
                RadioEvents->TxDone( );//调用 OnRadioTxDone
            }
        }
        ...
        if( ( irqRegs & IRQ_RX_DONE ) == IRQ_RX_DONE )
        {
            ...
            SX126xGetPayload( RadioRxPayload, &size , 255 );//获取payload所有数据
            SX126xGetPacketStatus( &RadioPktStatus );
            if( ( RadioEvents != NULL ) && ( RadioEvents->RxDone != NULL ) )//调用 OnRadioRxDone
            {
                RadioEvents->RxDone( RadioRxPayload, size, RadioPktStatus.Params.LoRa.RssiPkt, RadioPktStatus.Params.LoRa.SnrPkt );//解析RadioRxPayload size
            }
        }
        ...
        if( ( irqRegs & IRQ_CRC_ERROR ) == IRQ_CRC_ERROR )
        {
            ...
            {
                RadioEvents->RxError( );//调用OnRadioRxError
            }
        }
        ...
        if( ( irqRegs & IRQ_RX_TX_TIMEOUT ) == IRQ_RX_TX_TIMEOUT )
        {
            if( SX126xGetOperatingMode( ) == MODE_TX )
            {
                ...
                if( ( RadioEvents != NULL ) && ( RadioEvents->TxTimeout != NULL ) )
                {
                    RadioEvents->TxTimeout( );//调用OnRadioTxTimeout
                }
            }
            else if( SX126xGetOperatingMode( ) == MODE_RX )
            {
                ...
                if( ( RadioEvents != NULL ) && ( RadioEvents->RxTimeout != NULL ) )
                {
                    RadioEvents->RxTimeout( );//调用OnRadioRxTimeout
                }
            }
        }
