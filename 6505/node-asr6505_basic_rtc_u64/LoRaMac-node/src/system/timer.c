...
void TimerStart( TimerEvent_t *obj )
{
    ...
    {
     //   printf("test1 val %lu\r\n",obj->ReloadValue);
        TimerInsertNewHeadTimer( obj, obj->Timestamp );//¿Õº¯ÊýÍ¨¹ýµ÷ÓÃ´Ë´¦ÉèÖÃ³¬Ê±Ê±¼ä
    ...
    else
    {
        ...
        if( obj->Timestamp < remainingTime )//Èç¹ûÐÂ²åÈëµÄÊ±¼äÖµ Ð¡ÓÚÒªÖ´ÐÐµÄÍ·½áµãµÄÊ±¼äÖµ
        {
        //  printf("test2 %lu  %lu\r\n",obj->Timestamp,remainingTime);
            TimerInsertNewHeadTimer( obj, remainingTime );//ÉèÖÃ³¬Ê±Ê±¼ä
        }
        else
        {
      //    printf("test3\r\n");
...
static void TimerInsertNewHeadTimer( TimerEvent_t *obj, uint32_t remainingTime )
{
    ...
    if( cur != NULL )
    {
        cur->Timestamp = remainingTime - obj->Timestamp;//¼ÆËãÊ£ÏÂµÄÖµ¼õÈ¥²åÈëµÄÖµ ¾ÍÊÇ²åÈë¶¨Ê±Æ÷Ö´ÐÐÍê ÒªÖ´ÐÐÏÂÒ»¸ö¶¨Ê±Æ÷µÄÖµ
        cur->IsRunning = false;
    }

    obj->Next = cur;
    obj->IsRunning = true;
    TimerListHead = obj;//°ÑÒª²åÈëµÄÖµ·Åµ½¿ªÍ·
  // printf("new val :%d",TimerListHead->Timestamp);
    TimerSetTimeout( TimerListHead );//
   // printf("Rtc set timeout1\r\n");
}
//×÷ÓÃ£ºrtc¡ª¡ªalarm¶¨Ê±Ê±¼äµ½´¥·¢ TimerIrqHandler
//Í¨¹ýrtc¡ª¡ªalarm¶¨Ê±Æ÷´¥·¢¶¨Ê±Ê±¼äµ½µÄ»Øµ÷º¯Êý
//Í¬Ê±ÉèÖÃÏÂÒ»´Î³¬Ê±Ê±¼ä
void TimerIrqHandler( void )
{
    ...
    elapsedTime = TimerGetValue( );//»ñÈ¡ÄÖÖÓÔËÐÐÊ±¼ä
    ...
    while( ( TimerListHead != NULL ) && ( TimerListHead->Timestamp == 0 ) )
    {
        ...
        if( elapsedTimer->Callback != NULL )
        {
           // printf("time irq callback\r\n");//suqi
            elapsedTimer->Callback( );//Ö´ÐÐ»Øµ÷º¯Êý
        }
    }
    ...
    if( TimerListHead != NULL )
    {
        if( TimerListHead->IsRunning != true )
        {
            ...
            TimerSetTimeout( TimerListHead );//ÉèÖÃÏÂÒ»´Î³¬Ê±Ê±¼ä
...
//å¾—åˆ°å€¼ Sæˆ–è€…msçº§TICK  u32ä½  è¯»å–msä¸èƒ½ç”¨æ­¤å‡½æ•°
...
static void TimerSetTimeout( TimerEvent_t *obj )
{
   ...
   // obj->Timestamp = RtcGetAdjustedTimeoutValue( obj->Timestamp );//¶ÔÓÚ¶ÁÈ¡ÐÂµÄtick È¥µô²Å¸ü¾«´_ suqi
