...
bool RegionCN470ARxConfig( RxConfigParams_t *rxConfig, int8_t *datarate )
{
    ...
    if ( rxConfig->RxSlot == RX_SLOT_WIN_1 //rx接收窗口1
	#ifdef CONFIG_LINKWAN_D2D
	|| rxConfig->RxSlot == RX_SLOT_WIN_CLASS_D
	#endif
	) {
		...
		//printf("接收窗口1 rx frequency:%lld\r\n",frequency);
    }

    if (rxConfig->RxSlot == RX_SLOT_WIN_2) {//rx接收窗口2
        uint8_t uldl_mode;
	...
	#ifdef CONFIG_LINKWAN_D2D
    if(rxConfig->RxSlot == RX_SLOT_WIN_CLASS_D)
    {
        preambleLen = rxConfig->PreambleLen;
    }    
	#endif
    ...
    if ( dr == DR_7 ) {
        ...
        modem = MODEM_LORA;
        Radio.SetRxConfig( modem, rxConfig->Bandwidth, phyDr, 1, 0, preambleLen, rxConfig->WindowTimeout, false, 0, false, 0, 0,
    ...
    *datarate = (uint8_t) dr;
	...
	#endif
    {
        //DBG_LINKWAN("Rx, Freq %lu, DR %u, window %u\r\n", frequency, (uint16_t)dr, (uint16_t)rxConfig->RxSlot+1);
    }
...
bool RegionCN470ATxConfig( TxConfigParams_t *txConfig, int8_t *txPower, TimerTime_t *txTimeOnAir )
{
    ...
    if (txConfig->NodeWorkMode == WORK_MODE_REPEATER) {
        ...
        iqInverted = true;
		//printf("tx frequency1:%lld\r\n",frequency);
    } else {
        frequency = TxChannels[txConfig->Channel].Frequency;
        preambleLen = 8;
        iqInverted = false;
		//printf("tx frequency2:%lld\r\n",frequency);
    }
    ...
    *txPower = txConfig->TxPower;
    ...
    //            (int16_t)phyTxPower, (uint16_t)TxFreqBandNum, frequency, (int16_t)txConfig->Datarate, (uint16_t)txConfig->PktLen, *txTimeOnAir, curTime);
    return true;
}
    ...
    if (nextChanParams->AggrTimeOff <= TimerGetElapsedTime( nextChanParams->LastAggrTx)) {
        ...
        *aggregatedTimeOff = 0;
        ...
        nextTxDelay = RegionCommonUpdateBandTimeOff(nextChanParams->Joined, nextChanParams->DutyCycleEnabled, Bands,
                                                    CN470A_MAX_NB_BANDS);
        if(Bands[Channels[0].Band].TimeOff > 0){
            *time = nextTxDelay;   
            return true;
        }
    } else {
        nextTxDelay = nextChanParams->AggrTimeOff - TimerGetElapsedTime( nextChanParams->LastAggrTx );
        *aggregatedTimeOff = nextTxDelay;
        *time = nextTxDelay;
        return true;
    }
