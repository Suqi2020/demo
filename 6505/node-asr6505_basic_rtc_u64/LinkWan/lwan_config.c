...
int lwan_dev_config_get(int type, void *config)
{
    ...
    switch(type) {
        ...
        case DEV_CONFIG_CLASSD_PARAM:{
            memcpy(config,&g_lwan_dev_config.classd_param, sizeof(g_lwan_dev_config.classd_param));
            break;            
        }
		#endif        
...
int lwan_dev_config_set(int type, void *config)
{
    ...
    switch(type) {
		...
		#ifdef CONFIG_LINKWAN_D2D 
        case DEV_CONFIG_CLASSD_PARAM:{
            ClassDParam_t* classd_param = (ClassDParam_t* )config;
            if(classd_param->preambletime>15)
                return LWAN_ERROR;            
            memcpy(&g_lwan_dev_config.classd_param, config,sizeof(g_lwan_dev_config.classd_param));
            break;            
        }        
		#endif        
        case DEV_CONFIG_JOIN_SETTINGS: {          
