...
{
    RtcStartWakeUpAlarm( RtcConvertMsToTick( timeout ) );//// 参数带入ms
}
...
TimerTime_t RtcGetTimerValue( Tick_s_ms_enum tick )
{
    ...
    retVal = RtcConvertCalendarTickToTimerTime( NULL,tick );
...
RtcCalendar_t RtcConvertTimerTimeToCalendarTick( TimerTime_t timeCounter )
{
   ...
   uint32_t totleDaynum=0, totleSecNum=0;
    ...
    totleDaynum = unixTime/(SecondsInDay); //总天数(注意加括号)
    totleSecNum = unixTime%(SecondsInDay); //当天剩余的秒速
...
//suqi  加入  转换ms 和S tick选择 原函数转换位s
//函数作用 读取mstick 或者 stick
...
{
    double retVal = 0;
    retVal = round( (( ( double )timeoutValue * CONV_DENOM ) / CONV_NUMER) );//四舍五入
...
*/
     ...
     RTC_AlarmCmd(DISABLE);
    //disableInterrupts();
    RtcRecoverMcuStatus( );//初始化mcu
    RtcComputeWakeUpTime( );
    BlockLowPowerDuringTask( false );
    TimerIrqHandler( );
    TimerSetValue( &TxNextPacketTimer, 10000 );//唤醒后重新设置循环时间 
