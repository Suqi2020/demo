...
static void OnRadioTxDone( void )
{
    ...
    TimerTime_t curTime = TimerGetCurrentTime( TICK_MS );//ms tick ȥ����  �����иĶ�  suqi  Tick_s_ms_enum
    
    
#ifndef ADD_NEWCODE 
    LastTxSysTime.Seconds = TimerGetCurrentTime( TICK_S);
    LastTxSysTime.SubSeconds=TimerGetCurrentTime( TICK_MS)%1000  ;  //��Χ0-999 PREDIV_S
#endif
...
//#ifdef  SUQI
                    ...
                    sysTimeAns.SubSeconds = (int16_t)(sysTimeAns.SubSeconds * 3.90625); //一个字节扩大成2个字节
                    ...
                    TimerSetSysTime( sysTime );//设置系统时间

					
                    currentTime = TimerGetCurrentTime(TICK_MS );//获取u32 类型带ms——tick测试  测试  测试
                    
                    
                    LoRaMacClassBDeviceTimeAns( currentTime );//同步
#ifdef CONFIG_LWAN                    
...
#endif
                        LoRaMacClassBPingSlotInfoAns( );//��Ҫ
                    }
                }
                break;
