 ...
 */
//CLASS A &CºÏ²¢  ×¢Òâ:·Åµ½×ÔÖÆ¼þÖÐÐèÒªÐÞ¸Ä RFSWITCH ¾ßÌå²Î¿¼Íø¹Ø
//CONFIG_UART_DEBUG=0 //Ä¬ÈÏ 
//CONFIG_UART_CMD=1  //Ä¬ÈÏ
//Êý¾Ý·¢ËÍ ½ÓÊÕbuff   AppData[LORAWAN_APP_DATA_MAX_SIZE];
//tx_cmd_str[LORAWAN_APP_DATA_MAX_SIZE]
//sleep-low  rtcirq-high
//ÕýÊ½½»¸¶Ä£Ê½ TESTÐèÒªÆÁ±Î Í¬Ê±ÐèÒªÐÞ¸Ärfswitch
/*
...
int main( void )
{
  ...
  //  DeviceState = DEVICE_STATE_CYCLE;
    ...
    gu8Join_bz=1;//è°ƒè¯•classb è‡ªåŠ¨å…¥ç½‘ åŽæœŸåŽ»æŽ‰ suqi
#ifdef TIMETEST
    TimerInit( &TESTTimer, TESTEvent );

    TimerSetValue( &TESTTimer, 1000 );// TxDutyCycleTime
    TimerStart( &TESTTimer );
#endif
   
    
#if 0
while(1)//ÉÏµç½øÈëÅäÖÃÄ£Ê½
  {
    ...
    while( 1 )
    {
        switch( DeviceState )
        {
            case DEVICE_STATE_INIT:
            {
                ...
                mibReq.Param.Class = WORKMODE; //ÖØÐÂÅäÖÃ
		...
		LoRaMacCallbacks.GetTemperatureLevel= NULL;
                ...
                if(gu8Join_bz==1)//ATå‘½ä»¤ä¸­é…ç½®è¿‡workmode å¼€å§‹å…¥ç½‘       suqi
                      DeviceState = DEVICE_STATE_JOIN; //¿ªÊ¼ÈëÍø
                else
                      DeviceState = DEVICE_STATE_CYCLE;//»¹Î´ÈëÍø
                break;
            }
            case DEVICE_STATE_JOIN:
            {
             ...
             case DEVICE_STATE_REQ_DEVICE_TIME: {//»ñÈ¡ÉèÖÃÊ±¼ä beacon
                MlmeReq_t mlmeReq;
                ...
                if (mib_req.Param.IsNetworkJoined == true) {
                    if( NextTx == true ) {
                        mlmeReq.Type = MLME_DEVICE_TIME;//·¢ËÍready to send MOTE_MAC_DEVICE_TIME_REQ  »ñÈ¡Ê±¼ä
            ...
            case DEVICE_STATE_BEACON_ACQUISITION: {//·¢ping°ü ÇëÇó¶àÉÙÃëÒ»¸öÊ±Ï¶
                MlmeReq_t mlmeReq;
            ...
            case DEVICE_STATE_SEND:
            {
                ...
                //TxDutyCycleTime = APP_TX_DUTYCYCLE + randr( 0, APP_TX_DUTYCYCLE_RND );//²»·¢ËÍÊý¾ÝµÄ»° 5.7s»½ÐÑÒ»´Î
               DeviceState = DEVICE_STATE_CYCLE;
            ...
            case DEVICE_STATE_ATCMD://½øÈëATÖ¸ÁîÉèÖÃÄ£Ê½  DEVICE_STATE_CYCLE»á³¬Ê±Éè¶¨Ê±¼ä£¨0-60sÖ®¼ä£©ÍË³ö
