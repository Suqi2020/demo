...
static void OnRadioTxDone( void )
{
    ...
    TimerTime_t curTime = TimerGetCurrentTime( Tick_ms );//ms tick 去掉高  驱动有改动  suqi  Tick_s_ms_enum
    
    
#ifndef ADD_NEWCODE 
    LastTxSysTime.Seconds = TimerGetCurrentTime( Tick_s);
    LastTxSysTime.SubSeconds=TimerGetCurrentTime( Tick_ms)%1000  ;  //范围0-999 PREDIV_S
#endif
...
//#ifdef  SUQI
                    ...
                    sysTimeAns.SubSeconds = (int16_t)(sysTimeAns.SubSeconds * 3.90625); //转换成标准的ms
                    ...
                    sysTimeCurrent.Seconds = TimerGetCurrentTime( Tick_s);
                    sysTimeCurrent.SubSeconds=TimerGetCurrentTime( Tick_ms)%1000;  //范围0-999
                    ...
                    TimerSetSysTime( sysTime );//设置系统时间  suqi

					
                    currentTime = TimerGetCurrentTime(Tick_ms );//鑾峰彇u32 绫诲瀷甯s鈥斺�攖ick娴嬭瘯  娴嬭瘯  娴嬭瘯
                    
                    
                    LoRaMacClassBDeviceTimeAns( currentTime );//重要
#ifdef CONFIG_LWAN                    
...
#endif
                        LoRaMacClassBPingSlotInfoAns( );//重要
                    }
                }
                break;
                                       ...
                                       LoRaMacRegion_t region )
{
    ...
    LoRaMacInitializationTime = TimerGetCurrentTime(Tick_ms );
...
#ifdef CONFIG_LORA_VERIFY
	if (g_lora_debug) {
		mcps_start_time = TimerGetCurrentTime(Tick_ms );
	}
#endif
    if ( mcpsRequest == NULL ) {
        return LORAMAC_STATUS_PARAMETER_INVALID;
